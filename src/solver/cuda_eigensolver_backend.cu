                                    ...
                                    Eigen::MatrixXd& Q_total)
{
    ...
    for (int i = 0; i < n1; ++i)
        if (std::abs(T_subd(i)) <= eps * (std::abs(T_diag(i)) + std::abs(T_diag(i + 1))))
            T_subd(i) = 0.0;
...
static int check_convergence(int nev, const Eigen::VectorXd& ritz_val,
                             ...
                             double f_norm, double tol)
{
    const double eps23 = std::pow(std::numeric_limits<double>::epsilon(), 2.0 / 3.0);
    int nconv = 0;
    for (int i = 0; i < nev; ++i) {
    ...
    int nd, double sigma)
{
    ...
    auto run_lanczos = [&](int from_k, int to_m) -> int {
        ...
        for (int k = from_k; k < to_m; ++k) {
            ...
            if (k > 0) {
                for (int pass = 0; pass < 2; ++pass) {
                    ck(cublasDgemv(ctx_->cublas, CUBLAS_OP_T, n, k,
                                   &c1, d_W.ptr, static_cast<int>(stride),
                                   d_r.ptr, 1, &c0, d_co.ptr, 1),
                       "cublasDgemv(reorth T)");
                    ck(cublasDgemv(ctx_->cublas, CUBLAS_OP_N, n, k,
                                   &cn1, d_V.ptr, static_cast<int>(stride),
                                   d_co.ptr, 1, &c1, d_r.ptr, 1),
            ...
            if (beta_sq <= 0.0 || std::sqrt(beta_sq) < 1e-14) {
                nstep = k + 1;
                log_debug("[cuda-eig] lucky breakdown at step " +
                          std::to_string(k));
                break;
            }
            ...
            if (k < to_m - 1) {
                ...
                double inv = 1.0 / bk;
                ck(cublasDscal(ctx_->cublas, n, &inv, v_next, 1),
                   "cublasDscal(v)");
                ck(cublasDscal(ctx_->cublas, n, &inv, w_next, 1),
                   "cublasDscal(w)");
            }
        }
        return nstep;
    ...
    for (restart_iter = 0; restart_iter < maxit; ++restart_iter) {
        ...
        if (eig_T.info() != Eigen::Success)
            throw SolverError(
                ...
                ", restart=" + std::to_string(restart_iter) + ")");
        ...
        double f_norm = (nstep > 0 && nstep <= ncv)
            ...
            : 0.0;
        nconv = check_convergence(std::min(nd, nstep), ritz_val, ritz_est,
                                  f_norm, tol);
        ...
        if (nconv >= nd || nstep < ncv) break;  // converged or breakdown
        ...
        ck(cublasDgemm(ctx_->cublas, CUBLAS_OP_N, CUBLAS_OP_N,
                       n, k + 1, ncv,
                       &c1,
                       d_V.ptr, static_cast<int>(stride),
                       d_Q.ptr, ncv,
                       &c0,
        ...
        ck(cublasDgemm(ctx_->cublas, CUBLAS_OP_N, CUBLAS_OP_N,
                       n, k + 1, ncv,
                       &c1,
                       d_W.ptr, static_cast<int>(stride),
                       d_Q.ptr, ncv,
                       &c0,
                       d_temp.ptr, static_cast<int>(stride)),
        ...
        ck(cudaMemcpy(d_z.ptr, vk_ptr, stride * sizeof(double),
                      cudaMemcpyDeviceToDevice),
           "memcpy V[:,k]->z");
        ck(cublasDscal(ctx_->cublas, n, &h_kk1, d_z.ptr, 1),
           "cublasDscal(h_kk1)");
        // d_z += q_last * d_r
    ...
    // ── 9. Download V and compute Ritz vectors on CPU ───────────────────────
