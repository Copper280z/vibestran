...
TEST(ThermalSteady, CHexa8_VolumetricSource_FixedEndTemps) {
  ...
  bdf << "TEMP,2,1,0.0,2,0.0\n"
      << "TEMP,2,3,0.0,4,0.0\n";
  for (int n = 0; n < 4; ++n)
    bdf << "TEMP,2," << (1 + 4 * NEL + n) << ",0.0\n";
...
TEST(ThermalSteady, CQuad4_PlateInPlaneConduction) {
  ...
  bdf << "TEMP,1,1,200.0,2,200.0\n";
  bdf << "TEMP,1," << (1 + 2 * NEL) << ",0.0\n";
  bdf << "TEMP,1," << (2 + 2 * NEL) << ",0.0\n";
  bdf << "ENDDATA\n";
  const auto r = run_thermal(bdf.str());
...
TEST(ThermalSteady, Qvect_DirectionalFluxOnCHBDY) {
  ...
  // With q0=1000, k=10, L=1: T(z=1) should be 100.
  std::ostringstream bdf;
  bdf << "SOL 153\n"
      << "CEND\n"
      << "SUBCASE 1\n"
      ...
      << "ENDDATA\n";
  const auto r = run_thermal(bdf.str());
  // Top face nodes (5..8): expected T ≈ 100
  EXPECT_NEAR(temp_at(r, 5), 100.0, 1e-6);
  EXPECT_NEAR(temp_at(r, 7), 100.0, 1e-6);
}
...
TEST(ThermalSteady, CoupledHeatToStaticsBar) {
  ...
  bdf << "SOL 153\n"
      ...
      << "TEMP,1,3,100.0,4,100.0\n"
      << "TEMP,1,5,100.0,6,100.0\n"
      << "TEMP,1,7,100.0,8,100.0\n"
